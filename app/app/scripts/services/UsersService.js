	...
	function(User, UserPosts, UserRatings, $q, UserFollowers, UserFriends, UserFollowees, $analytics, CommunityMemberships, Notify) {
		...
		this.addFollower = function(user_id) {
			...
			UserFollowers.add({
				...
				Notify.addTranslate('NOTIFY.ADD_FOLLOWER_SUCCES', Notify.T_SUCCESS);
				return deferred.resolve(data);
			}, function(err) {
				
				Notify.addTranslate('NOTIFY.ADD_FOLLOWER_FAILED', Notify.T_ERROR);
				return deferred.reject(err);
			});
			return deferred.promise;
		};
		...
		this.removeFollower = function(user_id, followerId) {
			...
			UserFollowers.remove({
				...
				Notify.addTranslate('NOTIFY.REMOVE_FOLLOWER_SUCCES', Notify.T_SUCCESS);
				return deferred.resolve(data);
			}, function(err) {
				Notify.addTranslate('NOTIFY.REMOVE_FOLLOWER_FAILED', Notify.T_ERROR);
				return deferred.reject(err);
			});
			return deferred.promise;
		};
