...
angular.module('hearth.services').factory('Bubble', ['User', '$rootScope', 'Auth', '$analytics', '$state', '$compile', '$document', '$timeout',
	function(User, $rootScope, Auth, $analytics, $state, $compile, $document, $timeout) {
		...
		var bubbleDefinitions = {
			'bookmark-reminder': {
				applicable: function() {
					var confirmedForMoreThanAWeek = ($rootScope.loggedUser.confirmed_at && typeof $rootScope.loggedUser.confirmed_at.getTime === 'function' && $rootScope.loggedUser.confirmed_at.getTime() > 604800000);
					return (
