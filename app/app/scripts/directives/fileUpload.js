...
angular.module('hearth.directives').directive('fileUpload', [
	...
	function($timeout, $parse, $rootScope) {
		return {
			...
			replace: true,
			scope: true,
			template: '<div>' + '<span ng-transclude style="position:relative"></span>' + '<input class="file-upload-input" type="file" id="{{ uploadElementName }}" name="{{ uploadElementName }}" accept="image/*">' + '</div>',
