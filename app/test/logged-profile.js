describe('logged user profile', function() {  
  ...
  it('should go to my profile after select in menu', function () {
	...
	expect(element(by.css('h1>a>span')).getText()).toBe(protractor.helpers.config().loginFullName);
  });
  ...
  it('should go to edit profile after select in menu', function () {
	...
	expect(element(by.model('profile.name')).getAttribute('value')).toBe(protractor.helpers.config().loginFullName);
  });
  ...
  it('should be able to save profile', function () {
	...
	protractor.helpers.expectAlertBox('success');
  });
