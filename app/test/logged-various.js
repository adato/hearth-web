describe('logged user', function() {
    ...
    element.all(by.repeater('item in items')).then(function(items) {
       items.forEach(function (item, key) {
         ...
         buttonElement.click().then(function() {
            ...
            expect(element(by.css('.itemReply.hearth-modal')).isDisplayed()).toBe(true);
            element(by.css('.itemReply.itemEdit.hearth-modal .fleft.close')).click();
  ...
  it('should see create/edit dialog after "create new" click', function () {
    ...
    createButton.click();
    expect(element(by.css('.itemEdit.hearth-modal')).isDisplayed()).toBe(true);
    element(by.css('.itemEdit.hearth-modal a.fleft.close')).click();
    browser.driver.sleep(500);
    expect(element(by.css('.itemEdit.hearth-modal')).isPresent()).toBe(false);
  });
